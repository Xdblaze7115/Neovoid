...
TextureResource::TextureResource(const wchar_t* file_path) {
	if (!std::filesystem::exists(file_path)) {
		NEOLOG_ERROR("Texture file does not exist -> %s", std::filesystem::path(file_path).string().c_str());
		return;
	}
	...
	if (FAILED(hr)) {
		...
		NEOLOG_ERROR("Failed To Load Texture From File -> %s", str.c_str());
		return;
	}
	...
	std::wstring wstr(file_path);
	std::string str(wstr.begin(), wstr.end());
	NEOLOG_INFO("Successfully Loaded Texture -> %s (%dx%d)", str.c_str(), (int)scratch_image.GetMetadata().width, (int)scratch_image.GetMetadata().height);
}
